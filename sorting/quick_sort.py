...
# 1. select a pivot
# 2. partition the elements into two sub-arrays (less and greater than pivot)
...
def quicksort_in_place(arr, low, high):
    if low >= high:
        return arr
    # low and high in both functions are the exact index
    # (e.g. low = 0, high = len(arr) -1)
...
# this solution is for algorithm demostration
# it needs extra space but easy to understand
...
# use quicksort_in_place by default
def sort(arr):
    return quicksort_in_place(arr, 0, len(arr) - 1)
